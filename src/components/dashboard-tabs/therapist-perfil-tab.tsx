...
import { saveTherapistProfile, uploadVerificationDocuments } from "@/app/dashboard/actions";
...
export type PerfilTabParams = {
  perfil_error?: string;
  perfil_google_reconectado?: string;
  perfil_documentos_guardados?: string;
  perfil_guardado?: string;
};

// Contenido de la pestaña "Editar perfil" del panel de terapeuta — antes era
// la página completa /dashboard/perfil (ver ese archivo, ahora un redirect
// shim). Se mudó aquí el 2026-08-14 al fusionar los 5 destinos del
// terapeuta en un solo panel con pestañas (mismo criterio que el panel de
// admin). Server component puro: el botón "Guardar cambios" dispara el
// server action directo, sin JS de cliente.
export async function TherapistPerfilTab({ params }: { params: PerfilTabParams }) {
  const { perfil_error: error, perfil_google_reconectado: google_reconectado, perfil_documentos_guardados: documentos_guardados } = params;
  ...
  return (
    <div>
      ...
      {perfilGuardadoNotice(params.perfil_guardado)}
      ...
      {error === "suscripcion" && (
        <p className="mt-4 rounded-2xl border border-rose-deep/40 bg-rose/10 px-5 py-3 text-[0.9rem] text-rose-deep">
          ...
          tienes una suscripción activa. Suscríbete en la pestaña &ldquo;Suscripción&rdquo;.
        </p>
      )}
      {error === "foto" && (
        <p className="mt-4 rounded-2xl border border-rose-deep/40 bg-rose/10 px-5 py-3 text-[0.9rem] text-rose-deep">
      ...
      {therapist?.is_published && therapist?.slug && (
        <p className="mt-5 text-[0.85rem] text-[#7C877F]">
          <Link href={`/${therapist.slug}`} className="text-forest underline">
...
function perfilGuardadoNotice(perfil_guardado?: string) {
  if (perfil_guardado !== "1") return null;
  return (
    <p className="mt-4 rounded-2xl border border-line bg-forest/[0.06] px-5 py-3 text-[0.9rem] text-forest">
